...
def filtered_tickers(tickers):
    ...
    for t in tickers:
        try:
            # df = pyupbit.get_ohlcv(t, interval=minute, count=6)
            # print(df)
            # if df is None:
...
def trade_sell(ticker):
    ...
    df = pyupbit.get_ohlcv(ticker, interval = minute, count = 6)
    time.sleep(second)
    df_close = df['close'].values

    srsi_sell = 0.7 < srsi[1] > srsi[2] and 0.95 > srsi[2]
    count_upper_band = sum(1 for i in range(len(up_Bol)) if df_close[i] > up_Bol[i] * bol_upper_time)  #1.0025
    cut_upper_band = sum(1 for i in range(len(up_Bol)) if df_close[i] > up_Bol[i] * 1.01)  #1.0025
    ...
    if cut_start <= cut_time <= cut_end:      # 매도 제한시간이면
        ...
        if profit_rate >= min_rate:
            ...
            if middle_price:
                ...
                print(f"[m_price 미도달]: [{middle_price_time}][{ticker}] 수익률: {profit_rate:.2f}% / 현재가: {cur_price:,.1f} / srsi5: {srsi[1]:,.2f} > {srsi[2]:,.2f}")
                return None
        else:
...
#                     add_buy_cond = holding_value < add_buy_quant1 and profit_rate <= add_buy_rate1
