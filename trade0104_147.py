    ...
    if sell_start <= selltime <= sell_end:      # 매도 제한시간이면
        ...
        if profit_rate >= 0.6:  
            while attempts < max_attempts:
                ...
                if profit_rate >= 2 and current_price > up_Bol1 * 1.005 :
                    sell_order = upbit.sell_market_order(ticker, buyed_amount)
            ...
            if profit_rate >= 0.8 and df_15_high > up_Bol1*0.999 and srsi_k_1 > 0.8:
            # if profit_rate > 0.6 and current_price >= last_ema20 :
                ...
                send_discord_message(f"[매도시도 초과]: [{ticker}] 수익률: {profit_rate:.2f}% 현재가: {current_price:,.2f} srsi: {srsi_k_1:,.2f} > 0.8")
                print(f"[매도시도 초과]: [{ticker}] 수익률: {profit_rate:.2f}% 현재가: {current_price:,.2f} srsi: {srsi_k_1:,.2f} > 0.8")
                return sell_order   
            else:
                return None
    return None
    ...
    while True:
        try:
            ...
            for b in balances:
                ...
                if buyed_amount > 0:
                    profit_rate = (current_price - avg_buy_price) / avg_buy_price * 100 if avg_buy_price > 0 else 0  # 수익률 계산
                    report_message += f"[{b['currency']}] 수익률:{profit_rate:.1f}% s_RSI_K_15:{sRSI_K:.2f}\n"
    ...
    while True:
        ...
        for b in balances:
            if b['currency'] not in ["KRW", "QI", "ONX", "ETHF", "ETHW", "PURSE"]:  # 특정 통화 제외
                ...
                df_5_open = df_5['open'].iloc[-3:].tolist()  # 15분 봉 시가 리스트
                ...
                if profit_rate < -4 and any(Low_Bol[i] >= df_5_close[i] for i in range(3)) and all(df_5_open[i] > df_5_close[i] for i in range(3)):
                    if df_5_close[2] < current_price < Low_Bol[2] * 1.01:
                        if srsi_k_1 < 0.4:
                            krw = get_balance("KRW")
