...
def filtered_tickers(tickers):
    ...
    for t in tickers:
        try:
            ...
            is_increasing = all(band_diff[i] < band_diff[i + 1] * 1.0003 for i in range(len(band_diff) - 1))
...
def get_best_ticker():
    selected_tickers = ["KRW-BTC", "KRW-ETH", "KRW-XRP", "KRW-SOL", "KRW-ADA", "KRW-XLM", "KRW-DOGE", "KRW-HBAR", "KRW-AVAX", "KRW-sui",  "KRW-LINK"]  # 지정된 코인 리스트
    balances = upbit.get_balances()
    held_coins = []
    ...
    while True:
        try:
            ...
            for b in balances:
                ...
                if buyed_amount > 0:
                    profit_rate = (current_price - avg_buy_price) / avg_buy_price * 100 if avg_buy_price > 0 else 0  # 수익률 계산
                    report_message += f"[{b['currency']}] 현재가: {current_price:,.2f} 수익률: {profit_rate:.1f}% \n"
