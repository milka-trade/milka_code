...
def trade_buy(ticker, k):
    ...
    if krw >= 500_000 :  # 매수 조건 확인
        ...
        while attempt < max_retries:
                ...
                print(f"가격 확인 중: [{ticker}] 현재가:{current_price:,.2f} / < 목표가:{target_price:,.2f} / 0.2 < sRSI_D:{srsi_d:,.2f} < sRSI_K:{srsi_k:,.2f} < 0.35/ 시도:{attempt} - 최대:{max_retries}")
                # send_discord_message(f"매수시도: [{ticker}] 현재가:{current_price:,.2f} < 목표가:{target_price:,.2f} / 0.2 < sRSI_D:{srsi_d:,.2f} < sRSI_K:{srsi_k:,.2f} < 0.35 / 시도:{attempt} - 최대:{max_retries}")
                ...
                if current_price <= target_price and 0.1 < srsi_d < srsi_k < 0.4:
                    ...
                    for i in range(buy_attempts):
                        try:
                            ...
                            send_discord_message(f"매수 성공: [{ticker}] 현재가:{current_price:,.2f} < 목표가:{target_price:,.2f} / sRSI_D:{srsi_d:,.2f} < sRSI_K:{srsi_k:,.2f} / / 시도횟수:{attempt}")
                            return buy_order
...
def trade_sell(ticker):
    ...
    if not stoch_rsi_value.empty and len(stoch_rsi_value) >= 2:
        srsi_k = stoch_rsi_value['%K'].iloc[-1]
        pre_srsi_k = stoch_rsi_value['%K'].iloc[-2]
        # srsi_d = stoch_rsi_value['%D'].iloc[-1]
    ...
    if sell_start <= selltime <= sell_end:      # 매도 제한시간이면
        ...
        if profit_rate >= 0.6:  
            ...
            if profit_rate >= 0.6 and pre_srsi_k >= srsi_k :
            # if profit_rate > 0.6 and current_price >= last_ema20 :
                ...
                send_discord_message(f"[매도시도 초과]: [{ticker}] 수익률: {profit_rate:.2f}% 현재가: {current_price:,.2f} / pre_srsi_k:{pre_srsi_k:,.2f} >= srsi_k:{srsi_k:,.2f}")
                print(f"[매도시도 초과]: [{ticker}] 수익률: {profit_rate:.2f}% 현재가: {current_price:,.2f} / pre_srsi_k:{pre_srsi_k:,.2f} >= srsi_k:{srsi_k:,.2f}")
                return sell_order   
            else:
                return None
    return None
