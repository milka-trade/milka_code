...
class PortfolioManager:
    ...
    def sync_account(self) -> bool:
        ...
        try:
            ...
            # 주문가능금액이 있으면 우선 사용, 없으면 기존 방식(추정예탁자산 - 매입금액) 폴백
            order_available = summary.get("주문가능금액", "").strip()
            if order_available:
                self.available_cash = abs(int(order_available or "0"))
            else:
                self.available_cash = self.total_assets - self.total_invested
                logger_pf.warning(
                    "주문가능금액 조회 실패 — 추정예탁자산 기준으로 현금 계산."
                )

            logger_pf.info(
                f"현금 상세 | 주문가능금액:{self.available_cash:,} "
                f"| 추정예탁자산:{self.total_assets:,} "
                f"| 총매입금액:{self.total_invested:,}"
            )
