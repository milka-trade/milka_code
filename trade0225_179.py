...
def filtered_tickers(tickers):
    ...
    for t in tickers:
        try:
            ...
            is_increasing = all(band_diff[i] > 0.03 for i in range(len(band_diff) - 1))
...
def trade_sell(ticker):
    ...
    count_upper_band = sum(1 for i in range(len(up_Bol)) if df_close[i] > up_Bol[i] * 1.0025)  #1.0025
    ...
    if cut_start <= cut_time <= cut_end:      # 매도 제한시간이면
        ...
        if profit_rate >= min_rate:
            ...
            if profit_rate < cut_rate:
                sell_order = upbit.sell_market_order(ticker, buyed_amount)
                send_discord_message(f"[손절_CutRate]: [{ticker}] 수익률: {profit_rate:.2f}% / 현재가: {cur_price:,.1f} / srsi5: {srsi[1]:,.2f} > {srsi[2]:,.2f}")
            else:
                return None  
