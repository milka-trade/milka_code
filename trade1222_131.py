                    ...
                    if cur_price < day_open_price_1*1.02:                                    
                        ...
                        if last_ta_rsi < 60 :
                            ...
                            if df_15_low1 < Low_Bol1*1.005 or df_15_low2 < Low_Bol2*1.005 :
                                print(f"[cond 5]: [{t}] 볼린저밴드 하단 터치 / low1: {df_15_low1:,.2f} < bol1: {Low_Bol1:,.2f} / low2: {df_15_low2:,.2f} < bol2: {Low_Bol2:,.2f}")
                                # send_discord_message(f"[cond 5]: [{t}] 볼린저밴드 하단 터치 / low1: {df_15_low1:,.2f} < bol1: {Low_Bol1:,.2f} / low2: {df_15_low2:,.2f} < bol2: {Low_Bol2:,.2f}")
