...
def filtered_tickers(tickers, held_coins):
    ...
    for t in tickers:
        ...
        try:
            ...
            if day_value_1 > 10_000_000_000 and cur_price < day_open_price_1*1.01:
                ...
                if threshold_value < atr :
                    ...
                    # if cur_price < day_open_price_1*1.01:                                    
                        ...
                        if last_ta_rsi < 60 :
                            # print(f"[cond 3]: [{t}] [RSI]:{last_ta_rsi:,.2f} < 60")    

                            if df_15_close1 < Low_Bol1*1.001 or df_15_close2 < Low_Bol2*1.001 :
                                print(f"[cond 4]: [{t}] 15분 1봉 또는 2봉전에 볼린저밴드 하단 터치")
                                send_discord_message(f"[cond 4]: [{t}] 15분 1봉 또는 2봉전에 볼린저밴드 하단 터치")
                                            
                                if 0 < previous_ta_srsi < last_ta_srsi <= 0.2 and df_15_open < cur_price :
                                    print(f"[cond 5]: [{t}] 0 < [last s_RSI]:{last_ta_srsi:,.2f} <= 0.2 / 현재가 양봉")
                                    send_discord_message(f"[cond 5]: [{t}] [last s_RSI]:{last_ta_srsi:,.2f} <= 0.2 /  / 현재가 양봉")
                                    ...
                                    #     print(f"[cond 8]: [{t}] 현재가 양봉")
                                    #     send_discord_message(f"[cond 8]: [{t}] 현재가 양봉")
...
def trade_sell(ticker):
    ...
    if sell_start <= selltime <= sell_end:      # 매도 제한시간이면
        ...
        if profit_rate >= 0.5:  
            ...
            if profit_rate >= 0.8 and last_ema20*0.99 <= current_price:
                ...
                send_discord_message(f"[매도시도 초과]: [{ticker}] 수익률: {profit_rate:.2f}% 현재가: {current_price:,.2f} / 현재가가 ema 99% 이상")
                print(f"[매도시도 초과]: [{ticker}] 수익률: {profit_rate:.2f}% 현재가: {current_price:,.2f} / 현재가가 ema 99% 이상")
                return sell_order   
            else:
                return None
    return None
