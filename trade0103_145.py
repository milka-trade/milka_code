...
def filtered_tickers(tickers, held_coins):
    ...
    for t in tickers:
        ...
        try:
            ...
            df_15 = pyupbit.get_ohlcv(t, interval="minute15", count=4)
            if df_15 is None or df_15.empty:
                continue

            df_15_close = df_15['close'].iloc[-4:].tolist()  # 15분 봉 종가 리스트
            # print(f'{t} df_15_close[0]: {df_15_close[0]:,.2f} / df_15_close[1]: {df_15_close[1]:,.2f} / df_15_close[2]: {df_15_close[2]:,.2f}')
            df_15_open = df_15['open'].iloc[-4:].tolist()
            ...
            if cur_price < day_open_price_1*1.05 :
                ...
                # if threshold_value < atr:
                    ...
                    if Low_Bol[3] * 1.02 < up_Bol1 :
                        ...
                        if any(Low_Bol[i] >= df_15_close[i] for i in range(3)) and all(df_15_open[i] > df_15_close[i] for i in range(3)):
                            ...
                            if df_15_open[3] < df_15_close[3] and cur_price < Low_Bol[3] * 1.01:
                                print(f'[cond 4] {t} < 시가: {df_15_open[3]:,.2f} < 종가: {df_15_close[3]:,.2f} / 현재가: {cur_price:,.2f} < Low_Bol*1% : {Low_Bol[3]*1.01:,.2f}')
                                send_discord_message(f'[cond 4] {t} < 시가: {df_15_open[3]:,.2f} < 종가: {df_15_close[3]:,.2f} / 현재가: {cur_price:,.2f} < Low_Bol*1% : {Low_Bol[3]*1.01:,.2f}')
...
def trade_buy(ticker, k):
    ...
    if krw >= 500_000 :  # 매수 조건 확인
        ...
        while attempt < max_retries:
                ...
                if current_price <= target_price and 0 < srsi_k_1 < 0.4:
                    buy_attempts = 3
                    for i in range(buy_attempts):
                        try:
                            buy_order = upbit.buy_market_order(ticker, buy_size)
                            print(f"매수 성공: [{ticker}]")
                            send_discord_message(f"매수 성공: [{ticker}] 현재가:{current_price:,.2f} < 목표가:{target_price:,.2f} / sRSI_K:{srsi_k_1:,.2f} < 0.4 / / 시도횟수:{attempt}")
                            return buy_order
...
def trade_sell(ticker):
    ...
    df_15_high = df_15['high'].tail(1).iloc[-1]
    ...
    if sell_start <= selltime <= sell_end:      # 매도 제한시간이면
        ...
        if profit_rate >= 0.6:  
            ...
            if profit_rate >= 0.6 and df_15_high > up_Bol1 * 0.99 and srsi_k_1 > 0.8:
    ...
    while True:
        try:
            ...
            for b in balances:
                ...
                if buyed_amount > 0:
                    profit_rate = (current_price - avg_buy_price) / avg_buy_price * 100 if avg_buy_price > 0 else 0  # 수익률 계산
                    report_message += f"[{b['currency']}] 수익률:{profit_rate:.1f}% s_RSI_K:{sRSI_K:.2f}\n"
    ...
    while True:
        ...
        for b in balances:
            if b['currency'] not in ["KRW", "QI", "ONX", "ETHF", "ETHW", "PURSE"]:  # 특정 통화 제외
                ...
                df_15_close = df_15['close'].iloc[-4:].tolist()  # 15분 봉 종가 리스트
                # print(f'{t} df_15_close[0]: {df_15_close[0]:,.2f} / df_15_close[1]: {df_15_close[1]:,.2f} / df_15_close[2]: {df_15_close[2]:,.2f}')
                df_15_open = df_15['open'].iloc[-4:].tolist()  # 15분 봉 시가 리스트
                ...
                if profit_rate < -4 and any(Low_Bol[i] >= df_15_close[i] for i in range(3)) and all(df_15_open[i] > df_15_close[i] for i in range(3)):
                    if df_15_open[3] < df_15_close[3] and current_price < Low_Bol[3] * 1.01:
                        if srsi_k_1 < 0.4:
                            krw = get_balance("KRW")
                            ...
                            # 매수 결과 메시지 전송
                            if result:
                                send_discord_message(f"추가 매수: {ticker} / 수익률: {profit_rate:,.1f} / 수량: {buy_size} / 볼린저: {Low_Bol[3]:,.2f} / sRSI: {srsi_k_1:,.2f}")
                                print(f"추가 매수: {ticker} / 수익률: {profit_rate:,.1f} / 수량: {buy_size:,.1f} / 볼린저: {Low_Bol[3]:,.2f} / sRSI: {srsi_k_1:,.2f}")
