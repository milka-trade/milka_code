...
def filtered_tickers(tickers):
    ...
    for t in tickers:
        try:
            ...
            # band_diff = (upper_band[len(upper_band) - 1] - lower_band[len(upper_band) - 1]) / lower_band[len(upper_band) - 1]
            ...
            if ema_rising :
                ...
                if is_increasing :
                    ...
                    # if decreasing :
                    #     print(f'{t} 볼린저 기울기 완만:{decreasing}')
                # if lower_boliinger:
                    # print(f'{t} 볼린저 터치:{lower_boliinger}')
                    if srsi_buy1 :
    ...
    try:
        ...
        for ticker in all_tickers:
            ...
            # if ticker not in excluded_tickers or ticker in selected_tickers :
                ...
                # if day_value >= krw_sol_day_value and cur_price < day_price * 1.03:
...
def trade_buy(ticker):
    ...
    # low_price = (last_LBand < cur_price < last_LBand * 1.005)
...
def trade_sell(ticker):
    ...
    srsi_sell = 0.7 < srsi_k[2] < srsi_d[2]
    # count_upper_band = sum(1 for i in range(len(up_Bol)) if df_close[i] > up_Bol[i])  #1.0025
    cut_upper_band = sum(1 for i in range(len(up_Bol)) if df_close[i] > up_Bol[i] * 1.005)  #1.0025
    ...
    if cut_start <= cut_time <= cut_end:      # 매도 제한시간이면
        ...
        if profit_rate >= min_rate:
            ...
            if middle_price:
                sell_order = upbit.sell_market_order(ticker, buyed_amount)
                print(f"[m_price 도달]: [{ticker}] 수익률: {profit_rate:.2f}% / 현재가: {cur_price:,.1f} / srsi5: {srsi_d[2]:,.2f} > {srsi_k[2]:,.2f}")
                send_discord_message(f"[m_price 도달]: [{ticker}] 수익률: {profit_rate:.2f}% / 현재가: {cur_price:,.1f} / srsi5: {srsi_d[2]:,.2f} > {srsi_k[2]:,.2f}")
                return sell_order   
            else:
                middle_price_time = datetime.now().strftime('%m/%d %H:%M:%S')
                print(f"[m_price 미도달]: [{middle_price_time}][{ticker}] 수익률: {profit_rate:.2f}% / 현재가: {cur_price:,.1f} / srsi5: {srsi_d[2]:,.2f} > {srsi_k[2]:,.2f}")
                return None
        else:
            if profit_rate < cut_rate:
                sell_order = upbit.sell_market_order(ticker, buyed_amount)
                send_discord_message(f"[손절_CutRate]: [{ticker}] 수익률: {profit_rate:.2f}% / 현재가: {cur_price:,.1f} / srsi5: {srsi_d[2]:,.2f} > {srsi_k[2]:,.2f}")
            else:
                return None  
