...
def get_bollinger_bands(ticker, window=20, std_dev=2):
    ...
    # 상단 및 하단 밴드 값
    upper_band = bollinger.bollinger_hband().tail(6)  # 최근 6개의 상단 밴드 값
    lower_band = bollinger.bollinger_lband().tail(6)  # 최근 6개의 하단 밴드 값
            ...
            if day_value_1 > 20_000_000_000 and cur_price < day_open_price_1:
                ...
                if threshold_value < atr and up_Bol1 > Low_Bol1 * 1.03 :
                    ...
                    # print(f"[test]: [{t}] 볼밴 하단가 : {Low_Bol1:,.2f} > 현재가 : {cur_price:,.2f}")    
                        if Low_Bol3 >= df_15_close3 or Low_Bol2 >= df_15_close2 or Low_Bol1 >= df_15_close1 :
...
def trade_buy(ticker, k):
    ...
    if buyed_amount == 0 and krw >= 3_000_000 :  # 매수 조건 확인
        ...
        while attempt < max_retries:
                ...
                send_discord_message(f"매수시도: [{ticker}] 현재가:{current_price:,.2f} < 목표가:{target_price:,.2f} / pre_sRSI:{pre_srsi:,.2f} < lst_sRSI:{last_srsi:,.2f} < 0.3")
                if current_price <= target_price and 0.1 < pre_srsi < last_srsi < 0.3:
                    buy_attempts = 3
                    for i in range(buy_attempts):
                        try:
                            buy_order = upbit.buy_market_order(ticker, buy_size)
                            print(f"매수 성공: [{ticker}]")
                            send_discord_message(f"매수 성공: [{ticker}] 현재가:{current_price:,.2f} < 목표가:{target_price:,.2f} / pre_sRSI:{pre_srsi:,.2f} < lst_sRSI:{last_srsi:,.2f} < 0.3")
                            return buy_order
    ...
    while True:
        try:
            ...
            for b in balances:
                ...
                if buyed_amount > 0:
                    profit_rate = (current_price - avg_buy_price) / avg_buy_price * 100 if avg_buy_price > 0 else 0  # 수익률 계산
                    report_message += f"[{b['currency']}] 수익률:{profit_rate:.1f}% s_RSI:{last_sRSI:.2f}\n"
        ...
        if krw_balance > 1_000_000: 
            ...
            for b in balances:
                if b['currency'] not in ["KRW", "QI", "ONX", "ETHF", "ETHW", "PURSE"]:  # 특정 통화 제외
                    ...
                    if profit_rate < -4 and current_price < low_band * 0.99 and last_sRSI < 0.2 :      # 조건 체크: 수익률이 -4% 이하 현재가가 볼린저 밴드 하단 미만, sRSI 0.2미만
