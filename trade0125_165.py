...
def trade_buy(ticker):
    ...
    if krw >= min_krw :
        ...
        while attempt < max_retries:
            ...
            print(f"가격 확인 중: [{ticker}] lowPrice: {low_price} {last_df_open:,.2f} < {last_df_close:,.2f} / 현재가: {cur_price:,.2f} < {lower_band[len(lower_band) - 1] * 1.02:,.2f}/ 시도: {attempt} - 최대: {max_retries}")
            # send_discord_message(f"가격 확인 중: [{ticker}] lowPrice: {low_price} {last_df_open:,.2f} < {last_df_close:,.2f} / 현재가: {cur_price:,.2f} < {lower_band[len(lower_band) - 1] * 1.02:,.2f}/ 시도: {attempt} - 최대: {max_retries}")
            ...
            if low_price :
                ...
                for i in range(buy_attempts):
                    try:
                        ...
                        print(f"매수 성공: {ticker} / 시도횟수: {attempt}")
                        send_discord_message(f"매수 성공: {ticker} / 시도횟수:{attempt}")
                        return buy_order
...
def trade_sell(ticker):
    ...
    if profit_rate >= min_rate:
        ...
        if middle_price :
            sell_order = upbit.sell_market_order(ticker, buyed_amount)
            print(f"[매도시도 초과]: [{ticker}] 수익률: {profit_rate:.1f}% 현재가: {current_price:,.2f} ema: {last_ema20:,.2f} / srsi: {srsi[1]:,.2f} > {srsi[2]:,.2f}")
            send_discord_message(f"[매도시도 초과]: [{ticker}] 수익률: {profit_rate:.1f}% 현재가: {current_price:,.2f} ema: {last_ema20:,.2f} / srsi: {srsi[1]:,.2f} > {srsi[2]:,.2f}")
            return sell_order   
        else:
            return None
    return None  
    ...
    while True:
        ...
        for b in balances:
            if b['currency'] not in ["KRW", "QI", "ONX", "ETHF", "ETHW", "PURSE"]:  # 특정 통화 제외
                ...
                if profit_rate < profit_margin and krw > 50_000 and holding_value < buy_size * 3 :
                    ...
                    if is_downing and lower_boliinger and srsi_buy :
                        if low_price :
                            ...
                            if result:
                                ...
                                send_discord_message(f"추가 매수: {ticker} / 수익률: {profit_rate:,.1f} / 금액: {buy_size:,.0f} 볼린저하락: {is_downing} / 볼린저터치: {lower_boliinger} / srsi: {srsi_buy} / low_price:{low_price}")

                    else:
                        print(f'추매조건 미충족: {ticker} / 수익률: {profit_rate:,.1f} 볼린저하락: {is_downing} / 볼린저터치: {lower_boliinger} / srsi: {srsi_buy} / low_price:{low_price}')
                        # send_discord_message(f'추매조건 미충족: {ticker} / 수익률: {profit_rate:,.1f} \n 볼린저하락: {is_downing} / 볼린저터치: {lower_boliinger} / srsi: {srsi_k[1]:,.2f} < {srsi_k[2]:,.2f} / low_price:{low_price}')
                        time.sleep(second)
