                    ...
                    if is_downing and lower_boliinger and srsi_buy :
                        if low_price :
                            ...
                            if result:
                                ...
                                send_discord_message(f"추가 매수: {ticker} / 수익률: {profit_rate:,.1f} / 금액: {buy_size:,.0f}\n 볼린저하락: {is_downing} / 볼린저터치: {lower_boliinger} / srsi: {srsi_buy} {srsi_k[1]:,.2f} < {srsi_k[2]:,.2f} / low_price:{low_price}")

                    else:
                        print(f'추매조건 미충족: {ticker} / 수익률: {profit_rate:,.1f} 볼린저하락: {is_downing} / 볼린저터치: {lower_boliinger} / srsi: {srsi_buy} {srsi_k[1]:,.2f} < {srsi_k[2]:,.2f} / low_price:{low_price}')
