...
def get_user_input():
    ...
    trade_Qunat = simpledialog.askfloat("Input", "거래 금액을 입력하세요 (예: 1000000):")
    bol_touch_time = simpledialog.askinteger("Input", "볼린저 밴드 접촉 시간을 입력하세요 (예: 2):")
    min_rate = simpledialog.askfloat("Input", "최소 비율을 입력하세요 (예: 0.6):")
    max_rate = simpledialog.askfloat("Input", "최대 비율을 입력하세요 (예: 2.0):")
    profit_margin = simpledialog.askfloat("Input", "이익률을 입력하세요 (예: -4):")
    ...
    if sell_start <= selltime <= sell_end:      # 매도 제한시간이면
        ...
        if profit_rate >= min_rate:
            while attempts < max_attempts:
                ...
                if profit_rate >= max_rate and current_price > up_Bol[len(up_Bol)-1] * 1.01 and 0.8 < srsi[1] > srsi[2] :
                    sell_order = upbit.sell_market_order(ticker, buyed_amount)
            ...
            if profit_rate >= min_rate and current_price > last_ema20 * 1.01 and 0.8 < srsi[1] > srsi[2] :
                sell_order = upbit.sell_market_order(ticker, buyed_amount)
