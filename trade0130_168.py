...
def trade_sell(ticker):
    ...
    if profit_rate >= min_rate:
        ...
        if middle_price :
            ...
            send_discord_message(f"[매도시도 초과]: [{ticker}] 수익률: {profit_rate:.1f}% / middle_price: {middle_price} / srsi: {srsi_sell} {srsi[1]:,.2f} > {srsi[2]:,.2f}")
            return sell_order   
        else:
            return None
    return None  
                    ...
                    if is_downing and lower_boliinger and srsi_buy :
                        if low_price :
                            ...
                            if result:
                                ...
                                send_discord_message(f"[추가 매수]: {ticker} / 수익률: {profit_rate:,.1f} / 금액: {add_buy_size:,.0f}")
