...
def get_bollinger_bands(ticker, window=20, std_dev=2):
    ...
    # 상단 및 하단 밴드 값
    upper_band = bollinger.bollinger_hband().tail(4).fillna(0)  # 최근 4개의 상단 밴드 값
    lower_band = bollinger.bollinger_lband().tail(4).fillna(0)  # 최근 4개의 하단 밴드 값
    ...
    while True:
        try:
            ...
            for b in balances:
                ...
                if buyed_amount > 0:
                    profit_rate = (current_price - avg_buy_price) / avg_buy_price * 100 if avg_buy_price > 0 else 0  # 수익률 계산
                    report_message += f"[{b['currency']}] 수익률:{profit_rate:.1f}% s_RSI_K:{sRSI_K:.2f} > s_RSI_D:{sRSI_D}\n"
            ...
            for b in balances:
                if b['currency'] not in ["KRW", "QI", "ONX", "ETHF", "ETHW", "PURSE"]:  # 특정 통화 제외
                    ...
                    if profit_rate < -4 and current_price < low_band * 0.99 and 0.1 < sRSI_D < sRSI_K < 0.3 :      # 조건 체크: 수익률이 -4% 이하 현재가가 볼린저 밴드 하단 미만, sRSI 0.2미만
                        ...
                        # 매수 결과 메시지 전송
                        if result:
                            send_discord_message(f"추가 매수: {ticker} / 수익률 : {profit_rate:,.1f} / 볼린저*0.99 : {low_band*0.99:,.2f} / 현재가 : {current_price} / sRSI_D : {sRSI_D} < sRSI_K : {sRSI_K}")
