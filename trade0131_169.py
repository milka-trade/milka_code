...
add_buy_max = 3_000_000  # 추가매수 최대 금액 (3_500_000)
...
def get_user_input():
    global trade_Quant, bol_touch_time, min_rate, max_rate, sell_time
...
def trade_sell(ticker):
    ...
    srsi_sell_m = srsi[1] > srsi[2] and 0.9 > srsi[2] 
    ...
    if profit_rate >= min_rate:
        while attempts < max_attempts:
            ...
            print(f"[{ticker}] / [매도시도 {attempts + 1} / {max_attempts}] / 수익률: {profit_rate:.2f}% / up_price : {upper_price} / srsi: {srsi_sell} {srsi[1]:,.2f} > {srsi[2]:,.2f}") 
    ...
    while True:
        ...
        for b in balances:
            if b['currency'] not in ["KRW", "QI", "ONX", "ETHF", "ETHW", "PURSE"]:  # 특정 통화 제외
                ...
                elif (add_buy_quant2 < holding_value <= add_buy_quant3) and (profit_rate <= add_buy_rate3):
                    # add_buy_cond = True
                ...
                if add_buy_size > 0 and krw > min_krw and holding_value < add_buy_max :
