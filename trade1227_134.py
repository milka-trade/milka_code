...
def stochastic_rsi(ticker, smooth_k=3, smooth_d=3):
    ...
    # Stochastic RSI 값 반환
    # return stoch_rsi, k, d if not stoch_rsi.empty else 0
    return d if not stoch_rsi.empty else 0
...
def filtered_tickers(tickers, held_coins):
    ...
    for t in tickers:
        ...
        try:
            ...
            stoch_rsi = stochastic_rsi(t)   #스토캐스틱 RSI 계산
            ...
            if not stoch_rsi.empty and len(stoch_rsi) >= 2:
                ...
                previous_ta_srsi = stoch_rsi.iloc[-2]
            else:
                raise ValueError("stoch_rsi : Stochastic RSI data is insufficient.")

            last_srsi = stoch_rsi.iloc[-1]
            previous_srsi = stoch_rsi.iloc[-2]
...
def trade_buy(ticker, k):
    ...
    max_retries = 5  
    buy_size = min(1_500_000, krw*0.9995)
