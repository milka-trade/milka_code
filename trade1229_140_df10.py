...
def stoch_rsi(ticker):
    df = pyupbit.get_ohlcv(ticker, interval="minute10", count=50)
    time.sleep(0.5)
...
def filtered_tickers(tickers):
    ...
    for t in tickers:
        try:
            ...
            df_10 = pyupbit.get_ohlcv(t, interval="minute10", count=3)
            if df_10 is None or df_10.empty:
                continue
            
            df_10_close = df_10['close'].iloc[-3:].tolist()  # 15분 봉 종가 리스트
                ...
                if Low_Bol[0] * 1.02 < up_Bol1:
                    ...
                    if any(Low_Bol[i] >= df_10_close[i] for i in range(3)):
                        ...
                        if Low_Bol[0] < cur_price < Low_Bol[0] * 1.01:
                            print(f'[cond 4] {t} Low_Bol:{Low_Bol[0]:,.2f} < 현재가 : {cur_price:,.2f} < Low_Bol*1% : {Low_Bol[0]*1.01:,.2f}')
                            send_discord_message(f'[cond 4] {t} Low_Bol:{Low_Bol[0]:,.2f} < 현재가 : {cur_price:,.2f} < Low_Bol*1% : {Low_Bol[0]*1.01:,.2f}')
...
def trade_sell(ticker):
    ...
    if sell_start <= selltime <= sell_end:      # 매도 제한시간이면
        ...
        if profit_rate >= 0.6:  
            ...
            if profit_rate >= 0.6 and current_price > last_ema20*0.99:
                sell_order = upbit.sell_market_order(ticker, buyed_amount)
                send_discord_message(f"[매도시도 초과]: [{ticker}] 수익률: {profit_rate:.2f}% 현재가: {current_price:,.2f} / 현재가가 ema20 99% 이상")
                print(f"[매도시도 초과]: [{ticker}] 수익률: {profit_rate:.2f}% 현재가: {current_price:,.2f} / 현재가가 ema20 99% 이상")
                return sell_order   
            else:
                return None
    return None
